...
#
# Creates one partial from preface.adoc:
#   partials/purpose.adoc  – the Purpose section only
...
restructure_module() {
    ...
    # ---------------------------------------------------------------
    # 1. purpose.adoc  –  == Purpose … (stops before next == heading)
    # ---------------------------------------------------------------
    awk '
        /^== Preface[[:space:]]*$/ { next }
        /^== Purpose/              { in_purpose = 1 }
        in_purpose && /^== / && !/^== Purpose/ { exit }
        in_purpose { print }
    ' "$preface" > "$partials_dir/purpose.adoc"

    # ---------------------------------------------------------------
    # 2. pages/document_information.adoc  –  standalone last chapter
    # ---------------------------------------------------------------
    ...
    # Sections from preface (everything after Purpose, original == heading levels)
    awk '
        BEGIN { past_purpose = 0; in_doc = 0 }
        /^== Preface[[:space:]]*$/ { next }
        /^== Purpose/ { past_purpose = 1; next }
        past_purpose && !in_doc && /^== / { in_doc = 1 }
        past_purpose && !in_doc           { next }
        in_doc { print }
    ' "$preface" >> "$tmp_doc"
    ...
    # ---------------------------------------------------------------
    # 3. Rebuild index.adoc  –  header + Purpose only
    # ---------------------------------------------------------------
    local tmp_idx
    tmp_idx=$(mktemp)
